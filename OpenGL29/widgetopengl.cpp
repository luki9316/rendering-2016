...
void WidgetOpenGL::paintGL()
{
    ...
    {
        GLint FBO_screen;
        GLuint prog = shaderProgram;

        glGetIntegerv(GL_FRAMEBUFFER_BINDING, &FBO_screen);
        int attr_s = getUniformLocation(prog, "scale_slider");
        glUniform1i(attr_s, scale_slider);

        // rendering cienia
        glViewport(0, 0, SHADOW_FRAMEBUFFER_WIDTH / (scale_slider + 1), SHADOW_FRAMEBUFFER_HEIGHT / (scale_slider + 1));
        glBindFramebuffer(GL_FRAMEBUFFER, FBO);
        makeShadowMatrix();
        paintScene(true);
...
void WidgetOpenGL::move_scale_slider(int _slider)
{
    scale_slider = _slider;
